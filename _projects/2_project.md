...
Incompressible fluid flows, governed by the incompressible Navier-Stokes equations, are of considerable phenomenological richness. This includes forming complex spatio-temporal patterns, chaos, turbulence, and many others. 

Some of these observed phenomena could find elements of explanations by characterizing the linear response, i.e., the response to infinitesimally small disturbances, of the Navier-Stokes equations. <b> This typically includes computing the eigenvalues and eigenmodes of the linearised operator, since, at least in the limit  </b> $$t \rightarrow \infty$$, <b> the linear response of the flow to an initial perturbation will be structurally dominated by the eigenmodes associated with the least stable/most unstable eigenvalue </b>. Such analysis, concerned with the computation of eigenmodes, is referred to as being &ldquo; <b> modal </b> &rdquo;. 

While modal analyses of fluid flow are certainly relevant, the seminal work of <a href='https://www.science.org/doi/10.1126/science.261.5121.578'> Trefethen <i> et al. </i> </a> has also revealed that they are sometimes incomplete (even while remaining in a linear regime). Indeed, if $$u$$ denotes the velocity field of flow, the advective term $$(u \cdot \nabla ) u$$ present in the Navier-Stokes equations gives a preferential direction to the flow, and thus its linearisation makes the resulting operator <b> non-normal </b>. This means that the action of the linearised operator does not commute with that of its adjoint. <b> In that case, the finite-time flow response typically results from a complex interplay between a large number of eigenmodes </b>. Here, the restriction to the least stable/most unstable eigenmode structure is generically irrelevant. Furthermore, still at a finite time, a negative growth rate for all eigenvalues is not a guarantee for the energy to decay monotonically for all initial conditions:  <b> some small-amplitude initial perturbations may experience a considerable transient amplification </b>. <br> 
<b> The response of a non-normal system to a harmonic forcing is also typically associated with a substantial input/output amplification, much larger than the inverse of the smallest damping rate, and at forcing frequencies that can be completely different from the least stable eigenfrequencies </b>!<br>
Modal stability analyses have thus been generalised to said &ldquo; <b> nonmodal </b> &rdquo; ones, where singular modes of the operator mapping the input onto its output are found to furnish a much more efficient basis than its eigenmodes (hence the adjective &ldquo; nonmodal &rdquo; where &ldquo; non-eigenmodal &rdquo; should be understood)
...
Examples include the Holmboe instability, associated with relatively sharp density gradients, and giving rise to finite-amplitude propagating waves accompanied by vortices above and below the density interface. The Holmboe instability thus contributes to the mixing and transport of heat, salt, or indeed various pollutants. 

An important ingredient influencing Holmboe waves is the spatial confinement, inherent to many geophysical flows such as valleys, estuaries, submarine canyons, straits, or deep ocean trenches, as well as to lab experiments. <br>
However, laboratory observations in confined geometries are often compared to stability analyses that ignore confinement, and numerical simulations usually impose periodic boundary conditions. Instead, on the occasion of my Master's project at the University of Cambridge, François Gallaire, Adrien Lefauve, Colm-Cille Caulfield, and I have showed in {% cite Ducimetiere21 %} that the presence of rigid walls has a major impact on the stability of Holmboe waves in the experimental inclined square duct shown in figure 1.
...
<div class="row justify-content-sm-center">
    <div class="col-sm-9 mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/linho2.png" title="example image" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
...
Thin films flowing on substrates are ubiquitous both in nature and in industrial applications. Examples include lava flows on volcanoes, the spreading of paint, or spin-coating processes. These flows are typically driven by body forces, for instance, centrifugal or gravity, or surface-shear forces. They have in common the presence of a capillary ridge in the vicinity of the contact lines, the flow around which can be assumed to be quasi-static in most of the cases considered in the literature. In turn, the stationarity of flow here indeed justifies the resort to a modal analysis, which has revealed the ridge to be unstable in the spanwise direction, resulting in the formation of &ldquo; fingers &rdquo;. 

However, there are many flow configurations where the flow solution around the ridge cannot be considered as being quasi-static. This includes flows over curved surfaces, where the forces acting on the advancing ridge vary depending on its spatial location (see Fig. 3a). The generically fast time-dependency of the base flow there makes a modal analysis inappropriate. Indeed, the eigenmodes generically do not evolve exponentially in time if the base flow over which they have been computed itself is rapidly modified in time! 

A nonmodal analysis, on the other hand, supports any form of temporal analysis of the base flow. Considering a fluid spreading on a horizontal cylinder under the action of gravity (see Fig. 3a), <a href='https://scholar.google.com/citations?user=l3z_vhYAAAAJ&hl=en'> Gioele Balestra </a>, Mohamed Badaoui, François Gallaire, and I have thus conducted in {% cite Balestra19 %} an optimal transient growth (nonmodal) analysis to find out both the optimal perturbations of the initial film thickness and the cylinder topography (see Fig. 3b).
...
<div class="caption">
    Figure 4. Sketch of the iterative procedure to find the optimal gain and substrate perturbation for a given time horizon \( T \), spanwise wavenumber \( \beta \), Bond number \( Bo \), and film aspect ratio \( \delta \).
</div>

The latter algorithm provides the optimal azimuthal profile of the perturbation $$h^o$$, which, when applied to the cylinder topography, maximizes the transient gain
...
We found that, for all the considered parameters, the maximum possible transient gain (i.e., the ratio between the interfacial energy of the interface perturbation at some temporal horizon $$T$$, divided by that of the initial condition which seeded it) remained of $$O(1)$$ (see Fig. 6). Therefore, for this particular flow, the modal stability analysis seemed to provide a sufficiently complete description if the linear dynamics!
